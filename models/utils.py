...
def get_best_hyperparameters(train_x, train_y, llh_fn):
    """Get best hyperparameters using validation likelihood.
    
    Args:
        train_x: Training input data
        train_y: Training output data
        llh_fn: Likelihood function that takes (train_x, train_y, prior_var, noise_var)
    
    Returns:
        Tuple of (prior_var, noise_var) optimized for best likelihood
    """
    ...
    prior_var_candidates = [0.1, 0.5, 1.0, 2.0, 5.0]
    noise_var_candidates = [0.01, 0.05, 0.1, 0.2, 0.5]
    
    for prior_var in prior_var_candidates:
        for noise_var in noise_var_candidates:
            try:
                likelihood = llh_fn(train_x, train_y, prior_var, noise_var)
